...
<?php echo form_open('crud/updateSipil12')?>
	<input type="text" id="assessID" name="assessID" value="<?php echo $_GET['id']?>" hidden>
	<input type="text" id="table" name="table" value="assessmenttangga" hidden>
			...
			<table style="display:inline-block; text-align:left">
		      	...
		      	<tr>
		      		...
		      		<td><select id="select1" name="select1" class="form-control">
		      			...
		      			<option value="3">Minor / restorasi</option>ss
	...
	<div class ="panel-footer">
		...
		<input type="submit" name="save" value="SAVE & VIEW REPORT">
            ...
            success: function(data){
            	...
            	var json = jQuery.parseJSON(data);
              	$.each(json,function(i,item){
              		$('#select1 option[value='+item.struktur+']').attr('selected', 'selected');
              		$('#select2 option[value='+item.handrail+']').attr('selected', 'selected');
              		$('#select3 option[value='+item.anaktangga+']').attr('selected', 'selected');
              		$('#select4 option[value='+item.antislip+']').attr('selected', 'selected');
              		$('#select5 option[value='+item.suduttangga+']').attr('selected', 'selected');
              	});
