	...
	<center><h4 style='margin-top:0; margin-bottom:18px'>
		<?php if($_GET['status'] == 1){echo "Active Asset List";} 
				else if($_GET['status'] == 2){echo "Idle Asset List";}
				else if($_GET['status'] == 3){echo "Surplus Asset List";}
				else if($_GET['status'] == 4){echo "Scrapped Asset List";}
		?></h4>
	</center>
	...
	<tr>
		...
		<td id="tLoc" class="row10"><?php echo $isi->status ?></td>
	...
	<tr>
		...
		<td id="tLoc" class="row1"><?php echo $isi->status ?></td>	
		<td id="tAct"> 	
				<button type='button' class='openUpdate' data-toggle="modal" data-target="#myModal" data-id='<?php echo $isi->asset_id?>'><i class="fa faedit">Details</i></button> 
...
<div id="searchPanel" class="col-sm-2">
	<div id="searchBox">
	    	...
	    	<input type="text" name="jenisStatus" value="<?php echo $_GET['status'] ?>" hidden>
	    	<table>
