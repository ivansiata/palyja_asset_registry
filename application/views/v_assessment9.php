<br>
<center>
<h1>Proteksi Petir</h1>
<h3><span><?php echo $_GET['id']?></span>
	<?php echo $detail[0]->nama ?>
</h3>
</center>

<?php echo form_open('crud/updateSipil9')?>
	<input type="text" id="assessID" name="assessID" value="<?php echo $_GET['id']?>" hidden>
	<input type="text" id="table" name="table" value="assessmentproteksipetir" hidden>
            ...
            success: function(data){
            	...
            	var json = jQuery.parseJSON(data);
              	$.each(json,function(i,item){
              		$('#select1 option[value='+item.lightningrod+']').attr('selected', 'selected');
              		$('#select2 option[value='+item.konduktor+']').attr('selected', 'selected');
              		$('#select3 option[value='+item.koneksiatap+']').attr('selected', 'selected');
              		$('#select4 option[value='+item.koneksidinding+']').attr('selected', 'selected');
              		$('#select5 option[value='+item.groundrod+']').attr('selected', 'selected');
              		$('#select6 option[value='+item.sumurinspeksi+']').attr('selected', 'selected');
              	});
